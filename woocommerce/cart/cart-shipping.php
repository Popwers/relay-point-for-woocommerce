			...
			<ul id="shipping_method" class="woocommerce-shipping-methods">
				...
				if (count($available_methods) <= 1) {
					...
					if (count($WCRelay) > 0) {
						...
						printf('<input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s />', $index, esc_attr(sanitize_title($CurrentChecked)), esc_attr($CurrentChecked), checked($CurrentChecked, $chosen_method, false));
						...
						<script type="text/javascript">
							jQuery('.relay_wrapper input')[0].checked == false ? jQuery('.relay_wrapper select').attr('disabled',
									true) :
								jQuery(
									'.relay_wrapper select').removeAttr('disabled');
							...
							jQuery('.relay_wrapper select').on('change', function() {
								...
								jQuery('.relay_wrapper input').attr('name', name).attr('value', value).attr('data-index', data)
									.attr(
										'id', id);
